import { ReplayTrade, ReplaySummary, TradeOutcome, AdaptiveLearning } from '../types.js';
...
export function executeMemoryReplay(
  ...
  const skippedSetups: SkippedSetup[] = [];
  const blockedPatterns = new Set(activeLearnings.map(l => l.pattern_condition));
  ...
  for (let i = minWarmup; i < candles.length; i++) {
    ...
    if (!inPosition) {
      ...
      if (result.signal === 'BUY') {
        const pattern = classifyPattern(result.indicators);

        if (blockedPatterns.has(pattern)) {
          skippedSetups.push({
            time: new Date(currentCandle.timestamp).toISOString().replace('T', ' ').substring(5, 19),
            symbol,
            price: currentCandle.close,
            patternCondition: pattern,
            reason: `Blocked by Adaptive Memory: Pattern [${pattern}] historically caused losses.`
          });
          continue;
        }
  ...
  return { trades, skippedSetups, summary };
}
